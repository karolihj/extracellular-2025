// Run an EMI model simulation of two cerebellar purkinje neurons
...
int main(int argc, char *argv[])
{
    ...
    double Tdiff = 5.0;         // Difference between cells (in ms)
    ...
    args.AddOption(&Tdiff, "-td", "--Tdiff", "Difference between cells.");
    ...
    std::string simulation_name = "two_cells";
    if (U_dist) {
    ...
    int num_cells = 2;
    int dim = 3;
    const char *intracellular_mesh_file = "meshes/two_cells/intracellular.msh";
    const char *extracellular_mesh_file = "meshes/two_cells/extracellular.msh";
    std::string cell_base_file = "meshes/two_cells/cell";
    std::string load_mesh_string = "meshes/mesh_properties/two_cells_7branch_d2um";
   ...
   for (int i = 0; i < num_cells; i++)
   {
    ...
    mem_attr = 1;
    mem_attr[0] = 0; // Membrane nodes
    mem_attr[1] = 0; // Membrane nodes
    if (U_dist == false) {
    ...
    for (int i = 0; i < num_cells; i++) {
        ...
        A_d_large[i] = (*fg_m).InnerProduct(x_cell_d_large[i], x_cell_full[i])/A_full[i];
        A_d_medium[i] = (*fg_m).InnerProduct(x_cell_d_medium[i], x_cell_full[i])/A_full[i];
        A_d_small[i] = (*fg_m).InnerProduct(x_cell_d_small[i], x_cell_full[i])/A_full[i];
        A_soma[i] = (*fg_m).InnerProduct(x_cell_soma[i], x_cell_full[i])/A_full[i];
        A_AIS[i] = (*fg_m).InnerProduct(x_cell_AIS[i], x_cell_full[i])/A_full[i];
        A_pAIS[i] = (*fg_m).InnerProduct(x_cell_pAIS[i], x_cell_full[i])/A_full[i];
        A_RN[i] = (*fg_m).InnerProduct(x_cell_RN[i], x_cell_full[i])/A_full[i];
        A_collateral[i] = (*fg_m).InnerProduct(x_cell_collateral[i], x_cell_full[i])/A_full[i];
    }
    ...
    for (int i = 0; i < num_cells; i++)
    {
        for (int k=0; k<num_mem_nodes_cell[i]; k++) {
            ...
            if (i==1) {
                parameters_nodes[current_idx][Tdiff_idx] = Tdiff;
            }
            ...
            if (U_dist == false) {
                // Dendrite r > 4 um
                if (NU_nodes[current_idx] == 4) {
                    parameters_nodes[current_idx][Na_idx] = 0.016*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.0012*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 1.3e-4*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.01*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0.001*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 0.00001*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 3.5e-2*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 1.0e-3*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.002*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 1.0e-3*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 5.0e-6*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0012*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0001*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.000004*(A_d_large[0]/A_d_large[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_d_large[0]/A_d_large[i]);
                
                // Dendrite r > 1.75 um
                } else if (NU_nodes[current_idx] == 5) {
                    parameters_nodes[current_idx][Na_idx] = 0.0*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.0012*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 1.3e-4*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.01*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0.001*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 0.00001*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 3.5e-2*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 1.0e-3*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.002*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 1.0e-3*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 5.0e-6*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0012*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0001*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.000004*(A_d_medium[0]/A_d_medium[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_d_medium[0]/A_d_medium[i]);
                    
                // Dendrite r < 1.75 um
                } else if (NU_nodes[current_idx] == 6) {
                    parameters_nodes[current_idx][Na_idx] = 0.0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 30*0.0012*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 30*1.3e-4*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 30*0.01*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 30*0.001*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 30*0.0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 30*3.5e-2*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 30*0.0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 30*0.0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 1.0e-3*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 0.0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0001*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.000004*(A_d_small[0]/A_d_small[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_d_small[0]/A_d_small[i]);
                    
                // Soma
                } else if (NU_nodes[current_idx] == 7) {
                    parameters_nodes[current_idx][Na_idx] = 0.214*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.002*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 0.0*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.0*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0.05*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 3e-05*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 0.01*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 0.001*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.01*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 0.00022*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 7e-06*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0008*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0001*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.0004*(A_soma[0]/A_soma[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0011*(A_soma[0]/A_soma[i]);
                
                // AIS
                } else if (NU_nodes[current_idx] == 8) {
                    parameters_nodes[current_idx][Na_idx] = 1.5*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0.01*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 2.2e-4*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 1.0e-5*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_AIS[0]/A_AIS[i]);
                
                // Para AIS
                } else if (NU_nodes[current_idx] == 9) {
                    parameters_nodes[current_idx][Na_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.01*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_pAIS[0]/A_pAIS[i]);
                
                // Ranvier nodes
                } else if (NU_nodes[current_idx] == 10) {
                    parameters_nodes[current_idx][Na_idx] = 0.03*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0.01*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 2.2e-4*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 1.0e-5*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.0*(A_RN[0]/A_RN[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_RN[0]/A_RN[i]);
                
                // Collateral
                } else if (NU_nodes[current_idx] == 11) {
                    parameters_nodes[current_idx][Na_idx] = 0.03*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kv11_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kv15_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kv33_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kv34_idx] = 0.02*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kv43_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kir2x_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kca11_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kca22_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Kca31_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Cav21_idx] = 2.2e-4*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Cav31_idx] = 1.0e-5*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Cav32_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][Cav33_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][HCN1_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                    parameters_nodes[current_idx][leak_idx] = 0.0*(A_collateral[0]/A_collateral[i]);
                }
    ...
    // Update initial conditions
    for (int i=0; i < num_cells; i++) {
        x_cell_serial[i] = init_states[V_idx];
    }
    ...
    for (int n = 0; n < Nt; n++)
    {
        ...
        for (int k = 0; k < N_it; k++) {
            ...
            for (int m = 0; m < M_it; m++)
            {
                ...
                {
                    // Form linear system
                    a_cell[i]->FormLinearSystem(ess_tdof_list_cell[i], x_cell[i], *b_cell[i], A_i[i], X_i[i], B_i[i], 1);
            ...
            a_e->FormLinearSystem(ess_tdof_list_e, x_e, *b_e, A_e, X_e, B_e, 1);
