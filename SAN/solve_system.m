function [V_m, states_m, U_m] = solve_system(G)
% [V_m, states_m, U_m] = solve_system(G)
% Run a simulation of the bidomain model specified by the parameters G

% Set up mesh for the cells
mesh = set_up_mesh(G);
...
U_m(:,1) = U;
states_m = zeros(G.Nm, Ns, G.num_save);
states_m(:,:,1) = states';
...
for n = 1:G.Nt
    ...
    % Update the membrane potential in the membrane model
    states(G.V_idx, :) = V';
